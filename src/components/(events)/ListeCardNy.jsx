  ...
  return (
    <Link href={`/event/${event.id}`} className="group w-full cursor-pointer">
      <div className="relative aspect-[3/2]">
        <Image src={imageUrl} alt="statuebillede" width={300} height={200} className="w-full h-full object-cover transition duration-300 group-hover:bg-[#DAD6DD] group-hover:opacity-40 group-hover:border-[3px] group-hover:border-[#C4FF00]" />

        {/* BADGE: Vis kun hvis < 10 billetter */}
        {remainingTickets < 10 && <div className="absolute top-2 right-2 bg-gray-50 text-black text-xs font-bold px-2 py-1 z-10">Få billetter tilbage</div>}
